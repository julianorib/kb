...
- Um usuário convidado não pode usar o SSPR. A senha dele é originada do próprio tenant.
...
Para solucionar problemas de conectividade com um Load Balancer, utilize a Configuração da Investigação de Integridade.
...
Se houver arquivos com mesmo nome no Cloud Endpoint e no Server, não será apagado.\
Será mantido o arquivo que tiver data de modificação mais recente. O outro arquivo será renomeado para `<filename>-<conflict>-<GUID>.ext.`
...
- **O Recovery Services Vault deve estar na mesma região dos Recursos que estão sendo feitos Backups**
...
### Azure Site Recovery (ASR) ou Disaster Recovery
...
- **O Recovery Service Vault deve ficar na Região de destino dos Recursos que estão cobertos pelo DR**
